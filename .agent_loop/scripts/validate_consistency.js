...
function main() {
  ...
  const adj = new Map();
  for (const { base, full, rel } of relNames) {
    const content = fs.readFileSync(full, 'utf8');
    const refs = extractReferences(content, base);
    adj.set(base, refs.filter(r => baseNames.has(r)));
    adj.set('__rel_' + base, rel);
  }
  const cycles = findCycles(adj, baseNames);
  if (cycles.length > 0) {
    for (const c of cycles) {
      const rels = c.map(b => adj.get('__rel_' + b) || b);
      // Suppress warnings for intentional feedback loops:
      // - mutual_check agents cross-referencing each other (audit by design)
      // - user/planning ReAct cycle feedback loops
      const isMutualCheckLoop = rels.every(r => r.startsWith('safety-control/mutual_check/'));
      const isUserPlanningLoop = rels.every(r => r.startsWith('tooll_subagents/user/') || r.startsWith('tooll_subagents/planning/'));
      if (isMutualCheckLoop || isUserPlanningLoop) continue;
      const pathStr = rels.join(' -> ');
      warningCount++;
